...
type HighNodeUtilization struct {
	...
	args      *deschedulerconfig.HighNodeUtilizationArgs
...
func NewHighNodeUtilization(args runtime.Object, handle framework.Handle) (framework.Plugin, error) {
	highNodeUtilizatioArgs, ok := args.(*deschedulerconfig.HighNodeUtilizationArgs)
	if !ok {
		return nil, fmt.Errorf("want args to be of type HighNodeUtilizationArgs, got %T", args)
	}

	if err := validation.ValidateHighNodeUtilizationArgs(highNodeUtilizatioArgs); err != nil {
		return nil, err
	}
...
func (h *HighNodeUtilization) Balance(ctx context.Context, nodes []*v1.Node) *framework.Status {
	...
	targetThresholds := make(deschedulerconfig.ResourceThresholds)
...
func setDefaultForThresholds(thresholds, targetThresholds deschedulerconfig.ResourceThresholds) {
