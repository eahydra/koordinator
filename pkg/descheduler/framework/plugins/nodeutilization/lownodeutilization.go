...
type LowNodeUtilization struct {
	...
	args      *deschedulerconfig.LowNodeUtilizationArgs
...
func NewLowNodeUtilization(args runtime.Object, handle framework.Handle) (framework.Plugin, error) {
	lowNodeUtilizationArgsArgs, ok := args.(*deschedulerconfig.LowNodeUtilizationArgs)
	if !ok {
		return nil, fmt.Errorf("want args to be of type LowNodeUtilizationArgs, got %T", args)
	}

	if err := validation.ValidateLowNodeUtilizationArgs(lowNodeUtilizationArgsArgs); err != nil {
		return nil, err
	}
...
func (l *LowNodeUtilization) Balance(ctx context.Context, nodes []*v1.Node) *framework.Status {
	...
	continueEvictionCondFn := func(nodeInfo NodeInfo, totalAvailableUsage map[v1.ResourceName]*resource.Quantity) bool {
