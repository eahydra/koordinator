...
// RunReservationPreFilterExtensionRestoreReservation restores the Reservation during PreFilter phase
func (ext *frameworkExtenderImpl) RunReservationPreFilterExtensionRestoreReservation(ctx context.Context, cycleState *framework.CycleState, podToSchedule *corev1.Pod, matched []*ReservationInfo, unmatched []*ReservationInfo, nodeInfo *framework.NodeInfo) *framework.Status {
	for _, pl := range ext.reservationPreFilterExtensions {
		status := pl.RestoreReservation(ctx, cycleState, podToSchedule, matched, unmatched, nodeInfo)
		if !status.IsSuccess() {
			err := status.AsError()
			klog.ErrorS(err, "Failed running RestoreReservation on plugin", "plugin", pl.Name(), "pod", klog.KObj(podToSchedule))
			return framework.AsStatus(fmt.Errorf("running RestoreReservation on plugin %q: %w", pl.Name(), err))
		}
	}
	return nil
}
